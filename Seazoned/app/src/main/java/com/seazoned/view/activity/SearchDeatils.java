...
import com.seazoned.databinding.ActivitySearchDeatilsBinding;
...
import com.seazoned.view.adapter.SlidingImage_Adapter;
...
public class SearchDeatils extends AppCompatActivity implements View.OnClickListener {
    ActivitySearchDeatilsBinding mBinding;
    SearchDetailsRatingAdapter mAdapter;
    int currentPage;
    int NUM_PAGES;
    String landscaperId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_search_deatils);
        AppData.finalBookingParams = new HashMap<>();
        Util.setPadding(this, mBinding.mainLayout);
        mBinding.tvBookNow.setOnClickListener(this);
        mBinding.tvUpperBookNow.setOnClickListener(this);
        ...
        landscaperId = getIntent().getExtras().getString("landscaper_id");
    ...
    private void getSearchDetails(HashMap<String, String> params) {
        ...
        new PostDataParser(SearchDeatils.this, Api.sSearchDetails, params, true, new PostDataParser.OnGetResponseListner() {
            @Override
                        ...
                        if (success.equalsIgnoreCase("1")) {
                            ...
                            if (imageArray.length() > 0) {
                               ...
                               /* ArrayList<String> imageList = new ArrayList<>();
                                ...
                                mBinding.rcvImageSearch.setLayoutManager(new GridLayoutManager(SearchDeatils.this, 2));
                                mBinding.rcvImageSearch.setNestedScrollingEnabled(false);
                                mBinding.rcvImageSearch.setAdapter(image1Adapter = new RcvImageOneAdapter(SearchDeatils.this, imageList));
                                ...
                                image1Adapter.setOnItemClickListner(new RcvImageOneAdapter.OnItemClickListner() {
                                    ...
                                    public void onItemClick(int position) {
                                        Util.getPopUpWindow(SearchDeatils.this, imageList.get(position));
                            ...
                            /*else
                        {
                            ...
                            for (int i = 0; i < landscapperInfo.length(); i++) {
                                ...
                                if (!TextUtils.isEmpty(profile_image)) {
                                    Picasso.with(SearchDeatils.this).load(profile_image).error(R.drawable.icon_user).resize(200, 200).into(mBinding.ivProfileId);
                                } else {
                                    Picasso.with(SearchDeatils.this).load(R.drawable.icon_user).resize(200, 200).into(mBinding.ivProfileId);
                                }
                                if (!TextUtils.isEmpty(feature_image)) {
                                    mBinding.ivFeatureImage.setOnClickListener(new View.OnClickListener() {
                                        @Override
                                        public void onClick(View view) {
                                            Util.getPopUpWindow(SearchDeatils.this, feature_image);
                                        }
                                    });
                                    Picasso.with(SearchDeatils.this).load(feature_image).error(R.mipmap.featurepic).into(mBinding.ivFeatureImage);
                                } else {
                                    Picasso.with(SearchDeatils.this).load(R.mipmap.featurepic).into(mBinding.ivFeatureImage);
                                }
                            }
                            ...
                            mBinding.rcvRatingList.setAdapter(new SearchDetailsRatingAdapter(SearchDeatils.this, mRatingList));
    ...
    //coomented out by  kamalika

   /* private void setImage(ArrayList<String> images) {

        mBinding.vpImage.setAdapter(new SlidingImage_Adapter(SearchDeatils.this, images));
    ...
    public void onClick(View view) {
        if (view == mBinding.lnBack) {
            finish();
            //startActivity(new Intent(SearchDeatils.this, ServiceProviderActivity.class));
            overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
        } else if (view == mBinding.tvBookNow) {
            SharedPreferenceHelper helper=SharedPreferenceHelper.getInstance(SearchDeatils.this);
            if (helper.getUserLoginType().equalsIgnoreCase("guestLogin")){
                finish();//remove finish if stay here
                startActivity(new Intent(SearchDeatils.this, RegisterActivity.class)
                        .putExtra("type","guest")
                );
                overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);
            }
            else {
                AppData.finalBookingParams.put("landscaper_id", landscaperId);
                //AppData.finalBookingParams.put("service_price_id",);
                startActivity(new Intent(SearchDeatils.this, BookingStepOne.class)
                        .putExtra("addedServiceId", addedServiceId)
                        .putExtra("serviceId", serviceId)
                );
                overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);
            }

        } else if (view == mBinding.tvUpperBookNow) {
            mBinding.tvBookNow.performClick();
    ...
    private void editHeartFavorite(String url, HashMap<String, String> params) {
        new PostDataParser(SearchDeatils.this, url, params, false, new PostDataParser.OnGetResponseListner() {
            @Override
            public void onGetResponse(JSONObject response) {
                String success, msg;
                if (response != null) {
